...
public:
     ...
     */
    DynamicPGMIndexAsync() : used_levels(min_level), data(init_levels - min_level), pgm() {
    ...
    DynamicPGMIndexAsync(Iterator first, Iterator last) {
...
class DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>::DynamicPGMIndexIterator {
    friend class DynamicPGMIndexAsync;

    using internal_iterator = typename LevelType::const_iterator;
    using queue_pair = std::pair<internal_iterator, int16_t>;
    using dynamic_pgm_type = DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>;
...
template<typename K, typename V, typename SearchClass, typename PGMType, uint8_t MinIndexedLevel>
class DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>::BaseItemA {
protected:
    static V tombstone;
    V second;
...
template<typename K, typename V, typename SearchClass, typename PGMType, uint8_t MinIndexedLevel>
V DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>::BaseItemA::tombstone = new std::remove_pointer_t<V>();

template<typename K, typename V, typename SearchClass, typename PGMType, uint8_t MinIndexedLevel>
class DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>::BaseItemB {
protected:
    V second;
    bool flag;
...
template<typename K, typename V, typename SearchClass, typename PGMType, uint8_t MinIndexedLevel>
class DynamicPGMIndexAsync<K, V, SearchClass, PGMType, MinIndexedLevel>::Item : private BaseItem {
    friend class DynamicPGMIndexAsync;
    K first;
