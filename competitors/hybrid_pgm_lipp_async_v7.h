...
class HybridPGMLIPPAsync : public Competitor<KeyType,SearchClass> {
 ...
 public:
  ...
  uint64_t Build(const std::vector<KeyValue<KeyType>>& data,size_t){
    ...
    return util::timing([&]{
      ...
      if(base.size()>hybrid_detail::kMinBulk){
        ...
        lipp_.bulk_load(base.data(),base.size()); lipp_ready_.store(true);
  ...
  uint64_t EqualityLookup(const KeyType& key,uint32_t)const{
    ...
    if(lipp_ready_.load()){
      ...
      uint64_t v; if(lipp_.find(key,v)) return v;
    }
  ...
  uint64_t RangeQuery(const KeyType& lo,const KeyType& hi,uint32_t)const{
    lookup_ops_.fetch_add(1,std::memory_order_relaxed);
    uint64_t sum=0; std::unordered_set<KeyType> seen;
    {
      std::shared_lock dl(dpgm_m_);
      for(auto it=dpgm_.lower_bound(lo);it!=dpgm_.end()&&it->key()<=hi;++it){sum+=it->value();seen.insert(it->key());}
    }
    if(lipp_ready_.load()){
      std::shared_lock sl(lipp_m_);
 ...
 private:
  ...
  void merge_into_lipp(std::vector<KV>& b){
    ...
    if(!lipp_ready_.load()){
      if(b.size()>=hybrid_detail::kMinBulk) lipp_.bulk_load(b.data(),b.size());
      else for(auto &kv:b) lipp_.insert(kv.first,kv.second);
